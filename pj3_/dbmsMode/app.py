...
def s_edit():
    ...
    c_sid = request.args.get('sid')

    # print("sid::::",sid,"/")
    if c_sid==None:
        return render_template("s_add.html", head=head)
    c_sid = c_sid.replace(' ','')
    sql = f"SELECT sid,password,sname,sex,major_id,tutor_id,grade FROM students WHERE sid=\'{c_sid}\';"
    line = sqlQuery_(sql)
    print("01.",line)
    line = list(line[0])
    print("02.",line)
    return render_template("s_edit.html",head=head, con_data=line, sid=c_sid)
